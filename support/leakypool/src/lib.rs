...
impl<Element: 'static, TokenId: 'static> PoolPtr<Element, TokenId> {
    /// Convert a `PoolPtr` into a raw pointer.
    pub fn into_raw(self) -> NonNull<()> {
        NonNull::from(self.entry).cast()
    }

    /// Convert a raw pointer back into a `PoolPtr` .
    ///
    /// # Safety
    ///
    /// `ptr` must originate from `Self::into_raw`.
...
impl<Element> LeakyPool<Element, UncheckedToken> {
    ...
    /// # Safety
    ///
    /// This method allows the construction of multiple instances of `Self`.
    /// `PoolPtr`s generated by an instance of `Self` mustn't be used with
    /// another instance o `Self`. `UncheckedToken` doesn't pack information
    /// necessary for checking this at runtime.
    ///
    /// ```rust,no_compile
    /// use leakypool::LeakyPool;
    ///
    /// let mut pool1 = unsafe { LeakyPool::new_unchecked() };
    /// let mut pool2 = unsafe { LeakyPool::new_unchecked() };
    /// let ptr = pool1.allocate(42);
    ///
    /// // UB: Two mutable references pointing to the same object
    /// let ref1 = &mut pool1[ptr];
    /// let ref2 = &mut pool2[ptr];
    /// ```
