...
impl HView {
    ...
    pub(super) fn call_pending_mount_if_dirty(&self, wm: &WM) {
    ...
    fn call_pending_mount(&self, wm: &WM) {
        ...
        if !dirty.get().contains(ViewDirtyFlags::MOUNTED) {
            ...
            self.view.listener.borrow().mount(wm, self);
        }

        for subview in self.view.layout.borrow().subviews().iter() {
            subview.call_pending_mount(wm);
        }
    }
