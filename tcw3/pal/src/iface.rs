...
/// # Text input actions
///
/// The following actions are generated by the system when
/// [a text input context] is active:
///
///  - `SELECT_*`
///  - `*_WORD`
///  - `DELETE_*`
...
pub mod actions {
    ...
    iota::iota! {
        pub const UNDO: ActionId = SYS_START_VALUE + iota;
                ...
                , MOVE_START_OF_LINE
                , MOVE_END_OF_LINE
                , MOVE_LEFT_END_OF_LINE
                , MOVE_RIGHT_END_OF_LINE
                , MOVE_START_OF_PARAGRAPH
                , MOVE_END_OF_PARAGRAPH
                , MOVE_START_OF_DOCUMENT
                , MOVE_END_OF_DOCUMENT
                , MOVE_UP
                , MOVE_DOWN
                , MOVE_UP_PAGE
                , MOVE_DOWN_PAGE
                , MOVE_BACKWARD_SELECTING
                , MOVE_FORWARD_SELECTING
                , MOVE_LEFT_SELECTING
                , MOVE_RIGHT_SELECTING
                , MOVE_BACKWARD_WORD_SELECTING
                , MOVE_FORWARD_WORD_SELECTING
                , MOVE_LEFT_WORD_SELECTING
                , MOVE_RIGHT_WORD_SELECTING
                , MOVE_START_OF_LINE_SELECTING
                , MOVE_END_OF_LINE_SELECTING
                , MOVE_LEFT_END_OF_LINE_SELECTING
                , MOVE_RIGHT_END_OF_LINE_SELECTING
                , MOVE_START_OF_PARAGRAPH_SELECTING
                , MOVE_END_OF_PARAGRAPH_SELECTING
                , MOVE_START_OF_DOCUMENT_SELECTING
                , MOVE_END_OF_DOCUMENT_SELECTING
                , MOVE_UP_SELECTING
                , MOVE_DOWN_SELECTING
                , MOVE_UP_PAGE_SELECTING
                , MOVE_DOWN_PAGE_SELECTING
                , SCROLL_UP
                , SCROLL_DOWN
                , SCROLL_UP_PAGE
                , SCROLL_DOWN_PAGE
                , SCROLL_START_OF_DOCUMENT
                , SCROLL_END_OF_DOCUMENT
    }
}
...
pub trait TextLayout: Send + Sync + Sized {
    ...
    /// Find the first character boundary after or before the specified UTF-8
    /// offset.
    ///
    /// This method is intended for moving the insertion cursor during text
    /// editing.
    ///
    ...
    /// # Complexity
    ///
    /// The time complexity of this method is `O(text_len)`.
    ///
    /// # Rationale
    ///
    /// Finding grapheme boundaries requires [a moderately-sized table] and
    /// [a complex algorithm]. Most platforms provide a system library that does
    /// this, but it requires the input string to be given in a specific format.
    /// For example, in order to use
    /// `[NSString rangeOfComposedCharacterSequencesForRange:]`, one must
    /// first convert the input string to `NSString`. If we had a `TextLayout`,
    /// this step would be already done.
    ///
    /// One of TCW3's target platforms, GTK/GLib don't have facilities for
    /// finding grapheme boundaries. `GtkEntry` use text analysis data
    /// generated by Pango for cursor movement (there isn't much use of grapheme
    /// boundary information in other places of a GUI application). One could
    /// use Pango's API to generate this data (without layouting a text).
    /// However, it includes too much information to use just for this task.
    /// `PangoLayout` includes this infomation because it's needed for
    /// layouting. If we had `PangoLayout`, we could just borrow this
    /// information from that.
    ///
    /// Also, the rules for grapheme boundaries are occasionally revised. By
    /// relying on platform components, we can make sure that the application
    /// follows the latest rules without releasing a new version of the
    /// application. This is, of course, assuming that the application users
    /// keep their systems up-to-date.
    ///
    /// Furthermore, for some languages, grapheme boundaries aren't sufficient
    /// for determining valid cursor positions.
    ///
    /// Based on these points, I concluded that `TextLayout` is a good place to
    /// add this method.
    ///
    /// [a moderately-sized table]: https://github.com/unicode-rs/unicode-segmentation/blob/fbba2a660b38dad86eebcd7fe73138c5cbfb9469/src/tables.rs#L374-L957
    /// [a complex algorithm]: https://github.com/unicode-rs/unicode-segmentation/blob/fbba2a660b38dad86eebcd7fe73138c5cbfb9469/src/grapheme.rs
    fn next_char(&self, i: usize, forward: bool) -> usize;

    /// Find the first word boundary after or before the specified UTF-8 offset.
    ///
    /// This method is intended for moving the insertion cursor during text
    /// editing.
    ///
    /// It's unspecified whether this method skips whitespace characters
    ...
    /// # Rationale
    ///
    /// Similar to [`next_char`](TextLayout::next_char).
